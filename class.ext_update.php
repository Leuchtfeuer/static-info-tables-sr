...
class ext_update  {
	...
	function main()	{
		...
		if ($import == 'Import') {
			...
			$extPath = ExtensionManagementUtility::extPath('static_info_tables_sr');
			...
			foreach($fileContent as $line) {
				...
				if ($line && preg_match('#^UPDATE#i', $line) ) {
					...
					if (TYPO3_DLOG)
						GeneralUtility::devLog('SQL Query', 'static_info_tables_sr', 0, array('query:' => $query));
	 ...
	 *
	 * @param string $query Update statement like: UPDATE static_countries SET cn_short_sr='XXX' WHERE cn_iso_2='PT';
	...
	function getUpdateEncoded($query, $destEncoding) {
		...
		if (!($destEncoding==='utf-8')) {
			if(!is_object($srconv)) {
				$srconv = GeneralUtility::makeInstance('t3lib_cs');
			}
			...
			foreach ($queryFieldsArray as $fieldsSet) {
				...
				$value = $srconv->conv($value, 'utf-8', $destEncoding);
